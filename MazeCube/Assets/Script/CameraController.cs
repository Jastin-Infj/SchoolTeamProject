...
public class CameraController : MonoBehaviour {
    //プレイヤ追従カメラとマップビューカメラを切り替えるスクリプト
    //-------------------------------------------------------------------------------------
    ...
    void Start () {
            ...
            MapCamera.transform.position = (new Vector3(0, 20, 35)); //カメラの座標を指定
            MapCamera.transform.rotation = Quaternion.Euler(30, 180, 0); //カメラ向きを指定
    }
	
	// Update is called once per frame
	void Update () {
        if (Input.GetButtonDown("Fire3")) //spaceまたはジョイスティックボタン3（XboxコントローラーＹボタン）を押すと...
        {                                 //※使用する際はFire3のPositive Buttonをjoystick button 3に変更してください
        ...
        if (MapCamera.enabled == true) //マップカメラが有効な時のみ...
        {
            ...
            {                                 //※使用する際はFire1のPositive Buttonをjoystick button 4に変更してください
            ...
            {                                 //※使用する際はFire2のPositive Buttonをjoystick button 5に変更してください
                Cameracontroller.transform.Rotate(new Vector3(0, -45, 0));
            }
        }
    }
